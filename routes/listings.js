...
router.route('/')
    ...
    .post(isLoggedIn, validateListing, wrapAsync(listings.createListing)); // create listing

router.get('/new', isLoggedIn, listings.renderNewForm);  // new listing form

router.route('/:id')
    .get(wrapAsync(listings.showListing)) // Show listing details
    .put(isLoggedIn, isOwner, validateListing, wrapAsync(listings.updateListing)) // Update listing
